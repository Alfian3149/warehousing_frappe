...
export function ItemInfo({ onBack }: ItemInfoProps) {
  ...
  const handleScan = async (code: string) => {
    ...
    setLoading(true);
    setError(false);
    console.log(lotSerial);
    try {
      const response = await fetch(`/api/method/warehousing.warehousing.doctype.part_master.part_master.get_item_stock_details?item_code=${itemCode}`);
